...
def get_last_page_number(url, headers):
    """Determine the last page number for pagination with multiple fallback methods."""
    try:
        response = requests.get(url, headers=headers)
        soup = BeautifulSoup(response.text, 'html.parser')
        
        # Method 1: Try finding pagination elements
        pagination = soup.find('div', class_='pg-w')
        if pagination:
            pages = pagination.find_all('a', class_='pg')
            if pages:
                # Try to extract the last page number safely
                last_page_text = pages[-1].text.strip()
                if last_page_text:
                    try:
                        return int(last_page_text)
                    except ValueError:
                        logging.warning(f"Could not convert page number: {last_page_text}")
        
        # Method 2: Use regex to find page numbers in the HTML
        page_matches = re.findall(r'page=(\d+)', response.text)
        if page_matches:
            max_page = max(map(int, page_matches))
            if max_page > 1:
                return max_page
        
        # Method 3: Programmatically determine last page by trying subsequent pages
        test_page = 2
        while True:
            test_url = f"{url}?page={test_page}#catalog-listing"
            test_response = requests.get(test_url, headers=headers)
            test_soup = BeautifulSoup(test_response.text, 'html.parser')
            
            # Check if the page has any products
            if not test_soup.find_all('div', attrs={'data-catalog': 'true'}):
                return test_page - 1
        ...
        logging.warning("Could not determine last page number. Defaulting to 1.")
        return 1
    
    except Exception as e:
        logging.error(f"Error determining last page number: {e}")
        return 1
